    ...
    def validate_user(self, phone_number: str) -> Tuple[bool, str, Optional[User]]:
        ...
        try:
            ...
            if not user.google_sheets_id or not user.google_token:
                self.logger.warning(f"Incomplete Google Sheets configuration for user: {phone_number}")
                return False, "Configuração do Google Sheets incompleta. Por favor, configure suas credenciais.", None
